...
struct HistoryLineView: View {
    @State private var scrollPosition: CGPoint = .zero
    
    let history: [(CGFloat, Color)]
    let shift: Double
    @Binding var layout: UILayout
    ...
    init(history: [(CGFloat, Color)], shift: Double, layout: Binding<UILayout>) {
...
//        .border(.black, width: 5) // helps visualize how the scroll view works
        ...
        .overlay() {
            if history.count > 0 {
                PitchIndicator(x: (layout.indicatorX - layout.endOfAccidentals), y: layout.indicatorY)
                    .onChange(of: scrollPosition) {
                        layout.indicatorY = points[pointIndex].y
                    }
            }
        }
